...
Ordner `~/Library/Mobile Documents/com~apple~CloudDocs/Claude Code/Patience-Flow/` (seit 2026-08-23 in „Claude Code" verschoben; `migrateIcloudFolder()` in main.js zieht den alten Top-Level-Ordner selbst um) mit `data/Patience-Flow-Data.json` (gemeinsamer Datenstand, Backup-Format `{app,exported,data:{af_*}}`) und `app/Patience-Flow.html` (neueste App-Version).
